...
describe("field-registry", () => {
  ...
  it("formats primitive values for backend validation", () => {
    expect(formatFieldValueForSubmit({ type: "number" }, "42.5")).toBe(42.5);
    expect(formatFieldValueForSubmit({ type: "checkbox" }, false)).toBe(false);
  ...
  it("returns submit errors for values the backend would reject", () => {
    ...
    expect(
      getFieldValueSubmitError("items", { type: "relation_list" }, [
