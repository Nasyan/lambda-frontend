...
const getRelationItems = (value: JsonValue | undefined): RelationListItem[] => {
...
export function DynamicForm({
  ...
  const updateField = (fieldName: string, value: JsonValue) => {
    // Берем актуальный стейт напрямую из замыкания
    const next = { ...formData, [fieldName]: value };

    // Обновляем локальный стейт
    setFormData(next);

    // Сигнализируем наверх, но уже ВНЕ функции вычисления стейта
    onChange?.(next);
  };
              ...
              <div className="space-y-2">
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    while (
                      Object.prototype.hasOwnProperty.call(item, nextKey)
                    ) {
                      fieldIndex += 1;
    ...
    while (node) {
      ...
      controls.push(
        <label key={floorName} className="flex flex-col gap-1.5">
          <span className="text-xs font-medium text-gray-500">{floorName}</span>
          <select
            value={selectedValue}
            onChange={(event) =>
    ...
    const inputPlaceholder =
      renderer === "qr"
        ...
        : renderer === "geo_point"
          ...
          : renderer === "file_reference" || renderer === "image_reference"
