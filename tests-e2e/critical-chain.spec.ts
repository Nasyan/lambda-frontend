...
test("admin can reach invite-creator screen", async ({ page }) => {
  ...
  await expect(
    page.getByRole("heading", { name: "Пригласить креатора" }),
  ).toBeVisible();
...
test("admin instances screen exposes create + tables", async ({ page }) => {
  ...
  await expect(
    page.getByRole("heading", { name: "Управление инстансами" }),
  ).toBeVisible();
  await expect(
    page.getByRole("heading", { name: "Создать инстанс" }),
  ).toBeVisible();
...
test("register screen is invite-gated", async ({ page }) => {
  ...
  await expect(
    page.getByRole("heading", { name: "Регистрация по инвайту" }),
  ).toBeVisible();
});
...
test("instance user-management screen is reachable", async ({ page }) => {
  ...
  await expect(
    page.getByRole("heading", { name: "Пользователи и роли" }),
  ).toBeVisible();
  await expect(
    page.getByRole("heading", { name: "Пригласить пользователя" }),
  ).toBeVisible();
});
